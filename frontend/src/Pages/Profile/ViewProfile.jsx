...
function ViewProfile() {
  ...
  <>
  <Nav/>
  <div className='w-full sm:w-10/12 md:w-full xl:w-11/12 3xl:w-10/12 bg-gray-200 mx-auto py-2 sm:px-2 flex flex-col md:flex-row xs:relative xs:top-10 sm:top-12 md:top-14 lg:top-16 lg:h-screen-64 md:h-screen-56 sm:h-screen-2x-104 xs:h-screen-2x-40 h-screen-2x'>
    <div className='sm:w-11/12 md:w-1/2 lg:w-2/5 sm:h-screen-64 md:h-auto bg-yellow-500 md:mt-10 rounded-lg md:mr-2 relative group mx-auto'>
      ...
      <div className='px-2 w-full flex justify-between h-28'>
        <div className='flex flex-col items-start'>
         <h2 className='sm:text-xl md:text-lg lg:text-xl xl:text-2xl font-bold line-clamp-1 group-hover:line-clamp-none'>{profile.firstName} {profile.lastName && (profile.lastName)}</h2>
         <h2 className='sm:text-lg md:text-base xl:text-lg font-semibold text-gray-200'>@{profile.userName}</h2>
         <div className='flex sm:text-lg md:text-base lg:text-lg xl:text-xl'>
          {profile._id != userId && (
            <>
            {isFriend ? (
        ...
        <div className='flex flex-col items-end'>
         {profile.education && profile.education.college ? (
          <>
             <h2 className='sm:text-lg md:text-base xl:text-lg font-bold w-full line-clamp-1 group-hover:line-clamp-none text-right'>{profile.education.college.name}</h2>
             <h3 className='sm:text-base md:text-sm xl:text-base w-full font-semibold line-clamp-1 group-hover:line-clamp-none text-right'>{profile.education.college.major} {profile.education.college.year}</h3>
          </>
         ):(
          <>
          {profile.education && profile.education.school ? (
            <>
            <h2 className='sm:text-lg md:text-base xl:text-lg font-bold w-full line-clamp-1 group-hover:line-clamp-none text-right'>{profile.education.school.name}</h2>
            <h3 className='sm:text-base md:text-sm xl:text-base font-semibold line-clamp-1 group-hover:line-clamp-none text-right'>{profile.education.school.class}<sup>th</sup>Standard {profile.education?.school.year}</h3>
            </>
          ):(
            <></>
          )}
          ...
          <div className='flex sm:text-base md:text-sm xl:text-base font-bold'>
          {profile._id == userId && (
          <>
      ...
      <div className='w-full pt-2 pb-1 h-10 justify-between flex'>
         <div className='flex items-center font-bold sm:text-xl lg:text-2xl'>
           <button className='sm:px-2 cursor-default'>{profile.posts && (profile.posts.length)} Posts</button>
           <button className='sm:px-2 cursor-default'>{profile.friends && (profile.friends.length)} Friends</button>
         </div>
           {profile._id == userId && (
           <div className='relative sm:px-2 cursor-pointer' onMouseEnter={openDropdown} onMouseLeave={closeDropdown}>
           <SlMenu className='sm:text-xl lg:text-2xl font-bold'/>
           {isOpen && (
             <div className="absolute right-7 top-0 sm:w-40 xl:w-44 2xl:w-48 sm:text-base xl:text-lg 2xl:text-xl font-semibold bg-white border rounded-lg shadow-lg z-20">
      ...
      <div className='sm:text-lg lg:text-xl font-bold h-8 w-full text-center text-gray-700'>Friend List</div>
      <div className='w-full sm:h-screen-448 md:h-screen-496 lg:h-screen-504 px-2 pt-2 pb-4 overflow-y-auto flex flex-wrap' style={{scrollbarWidth:'none'}}>
        {friendList && friendList.map((item,index)=>{
          return(
            <div className='sm:w-1/3 md:w-1/3 lg:w-1/2 2xl:w-1/3' key={item._id}>
    ...
    <div className='sm:w-11/12 md:w-1/2 lg:w-3/5 flex relative lg:h-screen-120 md:h-screen-112 sm:h-screen-96 mx-auto'>
       <div className='sm:text-xl sm:py-2 md:text-lg lg:text-xl xl:text-2xl md:py-2 xl:py-1'>
